...
function getPersonByGitHub(githubId: number) {
  return pool.query(`SELECT pers.person_id, pers.name, oauth.oauth_id
                     FROM person pers
                     JOIN oauth_mapping oauth on oauth.person_id = pers.person_id 
                     WHERE oauth.oauth_id = $1`, [githubId]);
};
...
async function createPersonByGitHub(githubId: number) {
  ...
  try {
    const insertPersonText = "INSERT INTO person(name) VALUES('TEST') RETURNING person_id"
    const insertedPersonId = (await client.query(insertPersonText)).rows[0].person_id;
